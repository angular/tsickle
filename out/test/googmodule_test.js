...
const googmodule = require("../src/googmodule");
const modules_manifest_1 = require("../src/modules_manifest");
const testSupport = require("./test_support");
function processES5(fileName, content, { isES5 = true, isJsTranspilation = false, } = {}) {
    ...
    const host = {
        fileNameToModuleId: (fn) => path.relative(rootDir, fn),
        pathToModuleName: (context, fileName) => testSupport.pathToModuleName(rootDir, context, fileName),
        options,
    ...
    const diagnostics = [];
    const manifest = new modules_manifest_1.ModulesManifest();
    let output = null;
    const transformers = {
...
describe('convertCommonJsToGoogModule', () => {
    ...
    function expectCommonJs(fileName, content, isES5 = true) {
...
`);
    ...
    describe('processing transpiled JS output', () => {
        function expectJsTranspilation(content, filename = 'project/file.js') {
...
describe('resolveIndexShorthand', () => {
    let resolutionHost;
    const opts = Object.assign(Object.assign({}, testSupport.compilerOptions), { baseUrl: '/root', outDir: '/root/bin', rootDir: '/root', rootDirs: ['/root', '/root/gen'], paths: {
            '*': ['*', 'gen/*', 'bin/*'],
    ...
    function expectResolve(context, target) {
        const resolved = googmodule.resolveModuleName({ options: opts, moduleResolutionHost: resolutionHost }, context, target);
...
//# sourceMappingURL=googmodule_test.js.map