        ...
        return (sourceFile) => {
            ...
            function visitExportDeclaration(exportDecl) {
                ...
                if (importedModuleSymbol) {
                    ...
                    moduleTypeTranslator.requireType(exportDecl.moduleSpecifier, exportDecl.moduleSpecifier.text, importedModuleSymbol, 
                ...
                if (!exportDecl.exportClause) {
                    ...
                    exportDecl = ts.factory.updateExportDeclaration(exportDecl, exportDecl.decorators, exportDecl.modifiers, isTypeOnlyExport, ts.factory.createNamedExports(exportSpecifiers), exportDecl.moduleSpecifier, exportDecl.assertClause);
                }
                ...
                for (const [exportedName, sym] of typesToExport) {
                    ...
                    const stmt = ts.factory.createExpressionStatement(ts.factory.createPropertyAccessExpression(ts.factory.createIdentifier('exports'), exportedName));
                ...
                for (const decl of declNames) {
                    ...
                    if (!(0, transformer_util_1.symbolIsValue)(typeChecker, sym)) {
                        ...
                        if (node.kind === ts.SyntaxKind.ModuleDeclaration)
                            continue;
                        const mangledName = (0, annotator_host_1.moduleNameAsIdentifier)(host, sourceFile.fileName);
                        const declName = transformerUtil.getIdentifierText(decl);
                        const stmt = ts.factory.createExpressionStatement(ts.factory.createPropertyAccessExpression(ts.factory.createIdentifier('exports'), declName));
            ...
            function renameArrayBindings(node, aliases) {
                ...
                for (const e of node.elements) {
                    ...
                    if (ts.isArrayBindingPattern(e.name)) {
                        ...
                        // Plain identifier.
                        const aliasName = ts.factory.createIdentifier(`${e.name.text}__tsickle_destructured_${aliasCounter++}`);
                        aliases.push([e.name, aliasName]);
                        updatedBindingName = aliasName;
                    }
                    updatedElements.push(ts.factory.updateBindingElement(e, e.dotDotDotToken, ts.visitNode(e.propertyName, visitor), updatedBindingName, ts.visitNode(e.initializer, visitor)));
                }
            ...
            function createArrayBindingAliases(flags, aliases) {
                ...
                for (const [oldName, aliasName] of aliases) {
                    const typeStr = moduleTypeTranslator.typeToClosure(ts.getOriginalNode(oldName));
                    const closureCastExpr = ts.factory.createParenthesizedExpression(aliasName);
                    addCommentOn(closureCastExpr, [{ tagName: 'type', type: typeStr }], 
                    /* escape tags */ undefined, 
                    /* hasTrailingNewLine */ false);
                    const varDeclList = ts.factory.createVariableDeclarationList([ts.factory.createVariableDeclaration(oldName, /* exclamationToken? */ undefined, 
                        /* type? */ undefined, closureCastExpr)], flags);
            ...
            function visitForOfStatement(node) {
                ...
                const updatedInitializer = ts.factory.updateVariableDeclarationList(varDecls, [ts.factory.updateVariableDeclaration(varDecl, updatedPattern, varDecl.exclamationToken, varDecl.type, varDecl.initializer)]);
                ...
                if (ts.isBlock(node.statement)) {
                    ...
                    updatedStatement = ts.factory.createBlock([...aliasDecls, ts.visitNode(node.statement, visitor)]);
                }
                return ts.factory.updateForOfStatement(node, node.awaitModifier, updatedInitializer, ts.visitNode(node.expression, visitor), updatedStatement);
            }
            function visitor(node) {
                ...
                switch (node.kind) {
                    ...
                    case ts.SyntaxKind.FunctionExpression:
                        ...
                        return ts.factory.createParenthesizedExpression(visitFunctionLikeDeclaration(node));
                    case ts.SyntaxKind.Constructor:
                    case ts.SyntaxKind.FunctionDeclaration:
                    case ts.SyntaxKind.MethodDeclaration:
                    case ts.SyntaxKind.GetAccessor:
...
//# sourceMappingURL=jsdoc_transformer.js.map