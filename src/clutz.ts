...
export function makeDeclarationTransformerFactory(
  ...
  host: ClutzHost & googmodule.GoogModuleProcessorHost,
    ...
    return {
      ...
      transformSourceFile(file: ts.SourceFile): ts.SourceFile {
        ...
        const globalBlock = generateClutzAliases(
          file,
          host.pathToModuleName('', file.fileName),
          typeChecker,
          options,
        );
        ...
        return ts.factory.updateSourceFile(
          ...
          file.referencedFiles.map((f) =>
            fixRelativeReference(f, file, options, host),
          ),
...
function generateClutzAliases(
  ...
  for (const symbol of localExports) {
    ...
    const declaration = symbol.declarations?.find(
      (d) => d.getSourceFile() === origSourceFile,
    );
    if (
      declaration &&
  ...
  const globalDeclarations: ts.Statement[] = [
    ...
    ts.factory.createExportDeclaration(
      /* modifiers */ undefined,
      /* isTypeOnly */ false,
      ts.factory.createNamedExports(globalExports),
    ),
    // 2) For nestedExports
...
function gatherNecessaryClutzImports(
  ...
  for (const stmt of sf.statements) {
    ...
    const moduleSymbol = ambientModuleSymbolFromClutz(
      googmoduleHost,
      typeChecker,
      stmt,
    );
