...
import {getIdentifierText, hasModifierFlag, isAmbient, isDtsFileName} from './transformer_util';
...
/**
 * Returns true if the given file is one that we believe tsickle will control
 * the emit for.  In particular, this matters for knowing whether tsickle has
 * mangled a type name.
 */
function isTsickleProcessedFile(sourceFile: ts.SourceFile): boolean {
  if (isDtsFileName(sourceFile.fileName)) {
    // d.ts files generated by tsickle contain the tsickle header.
    return sourceFile.text.startsWith(TSICKLE_HEADER_COMMENT);
  }

  // Otherwise, any .ts file we see as part of compilation will be tsickle-processed.
  return true;
}

/**
 * Returns true if the given symbol is one where tsickle renamed the symbol
 * when emitting for Closure output.
 */
function isTsickleRenamedType(sym: ts.Symbol): boolean {
  // Tsickle only renames types that are also values.
  if (!(sym.flags & ts.SymbolFlags.Value)) return false;
  // Tsickle doesn't rename classes or enums.
  if (sym.flags & (ts.SymbolFlags.Class | ts.SymbolFlags.Enum)) return false;

  // Tsickle only renames types that it controls the emit for.
  // Note: use declarations.every(), because if *any* file affecting the type
  // isn't tsickle-processed, then tsickle cannot have renamed it.  This comes up
  // when someone e.g. amends a browser built-in type from a tsickle-processed file.
  return sym.declarations != null &&
      sym.declarations.every(n => isTsickleProcessedFile(n.getSourceFile()));
}
...
export class TypeTranslator {
  ...
  symbolToString(sym: ts.Symbol): string|undefined {
    ...
    // For some symbols, when emitting to Closure we mangled the type
    // name to avoid a conflict.  Use the mangled name here.
    // See the type_and_value test.
    if (isTsickleRenamedType(sym)) {
      str += '$$TSType';
    }
