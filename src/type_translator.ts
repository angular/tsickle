...
export class TypeTranslator {
  ...
  constructor(
    ...
    private readonly pathUnknownSymbolsSet: Set<string>,
    ...
    this.pathUnknownSymbolsSet = new Set<string>(
  ...
  translate(type: ts.Type): string {
    ...
    switch (type.flags & mask) {
      ...
      default:
        ...
        // Note: An enum is typed as a union with TS 5.0. This means
        // multiple flags are present on enum types. `EnumLike` matches these.
        if (type.flags & ts.TypeFlags.EnumLike) {
          if (!type.symbol) {
            this.warn(`EnumLike without a symbol`);
            return '?';
          }
          // In TS5.0, enum types are represented as a union of enum literals.
          // Each literal in the enum is a ts.TypeFlags.EnumLiteral. If the
          // symbol is marked as being an enum member, translate it by
          // referencing the owning enum container.
          if (type.symbol.flags & ts.SymbolFlags.EnumMember) {
            return this.translateEnumLiteralMember(type);
          }

          const typeStr = this.symbolToString(type.symbol);
          return typeStr !== undefined ? `!${typeStr}` : '?';
        }

        // Types with literal members are represented as
        //   ts.TypeFlags.Union | [literal member]
        // E.g. A boolean type is a union type with 'true' and 'false' as its members.
  ...
  private translateEnumLiteralMember(type: ts.Type): string {
    // Suppose you had:
...
export function isAlwaysUnknownSymbol(
  pathUnknownSymbolsSet: Set<string> | undefined,
  symbol: ts.Symbol,
) {
