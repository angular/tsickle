...
export class DecoratorClassVisitor {
  ...
  ctorParameters: Array<[[ts.TypeNode, string] | undefined, ts.Decorator[]|undefined]|null>;
  ...
  private gatherConstructor(ctor: ts.ConstructorDeclaration) {
    const ctorParameters:
        Array<[[ts.TypeNode, string] | undefined, ts.Decorator[] | undefined]|null> = [];
    ...
    if (this.decorators || this.ctorParameters) {
      ...
      for (const param of this.ctorParameters || []) {
        ...
        if (!ctor) {
          ...
          const [typeNode, typeStr] = ctor;
          const emitNode = findTypeNameNodeWithRange(
              typeNode, typeNode.getStart(), typeNode.getStart() + typeStr.length);
          if (emitNode && emitNode.getText() === typeStr) {
...
function findTypeNameNodeWithRange(node: ts.Node, start: number, end: number): ts.Node|undefined {
  if (node.getStart() === start && node.getEnd() === end) {
    return node;
  }
  return ts.forEachChild(node, (child) => findTypeNameNodeWithRange(child, start, end));
}
