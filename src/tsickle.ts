...
import {DecoratorClassVisitor, visitClassContent} from './decorator-annotator';
...
export interface Options {
  ...
  /**
   * Whether to downlevel decorators as well.
   */
  transformDecorators?: boolean;
  /**
   * Whether to filter out exports for types when expanding an `export * ...`.
   * Needed e.g. for transformer mode, as
   * the typechecker has no information of the new symbols,
   * and therefore typescript won't automatically remove them.
   */
  filterTypesForExport?: boolean;
}
...
class ClosureRewriter extends Rewriter {
  ...
  constructor(
      protected typeChecker: ts.TypeChecker, protected host: Host, protected options: Options,
      file: ts.SourceFile, sourceMapper?: SourceMapper) {
  ...
  newTypeTranslator(context: ts.Node) {
    const translator = new typeTranslator.TypeTranslator(
        this.typeChecker, ts.getOriginalNode(context), this.options.typeBlackListPaths,
...
class Annotator extends ClosureRewriter {
  ...
  constructor(
      typeChecker: ts.TypeChecker, host: Host, options: Options, file: ts.SourceFile,
      sourceMapper?: SourceMapper, private tsHost?: ts.ModuleResolutionHost,
      private tsOpts?: ts.CompilerOptions) {
    super(typeChecker, host, options, file, sourceMapper);
  }
  ...
  private shouldEmitExportSymbol(sym: ts.Symbol): boolean {
    if (!this.options.filterTypesForExport) {
      return true;
    }
    if (sym.flags & ts.SymbolFlags.Alias) {
  ...
  private emitTypeDefExports(exports: NamedSymbol[]) {
    ...
    for (const exp of exports) {
      ...
      if (this.options.filterTypesForExport) {
        // If we are in the transformer pipeline,
        // we don't know that tsickle converted interfaces into functions,
        // so we have to manually reexport them as well.
  ...
  private visitClassDeclaration(classDecl: ts.ClassDeclaration) {
    ...
    if (this.options.transformDecorators) {
    ...
    visitClassContent(classDecl, this, this.currentDecoratorConverter);
    this.writeRange(classDecl, classDecl.getEnd(), classDecl.getEnd());
...
class ExternsWriter extends ClosureRewriter {
  ...
  public visit(node: ts.Node, namespace: string[] = []) {
    switch (node.kind) {
      ...
      case ts.SyntaxKind.FunctionDeclaration:
        ...
        if (ts.getOriginalNode(fnDecl) !== decls[0]) break;
  ...
  private isFirstDeclaration(decl: ts.DeclarationStatement): boolean {
    ...
    return ts.getOriginalNode(decl) === sym.declarations[0];
  }
...
export function annotate(
    typeChecker: ts.TypeChecker, file: ts.SourceFile, host: Host, options: Options = {},
  ...
  return new Annotator(typeChecker, host, options, file, sourceMapper, tsHost, tsOpts).annotate();
}

export function writeExterns(
    typeChecker: ts.TypeChecker, file: ts.SourceFile, host: Host,
    options: Options = {}): {output: string, diagnostics: ts.Diagnostic[]} {
  typeTranslator.assertTypeChecked(file);
  return new ExternsWriter(typeChecker, host, options, file).process();
}
