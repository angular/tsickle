 ...
 *
 * Note: we don't have suppress {duplicate} here but rather on the variable statement
 * on purpose. See SUPPRESS_DUPLICATE_TAG.
 *
 * TODO(tbosch): actually remove `duplicate` from this header.
 * See https://github.com/angular/tsickle/issues/619.
 */
...
/**
 * Always suppress duplicate definitions at the variable declaration,
 * and not at the file level.
 * If the @suppress is at the file level, then it doesn't affect other
 * instances of the variable in other files, while apparently
 * applying it to the variable does.
 */
const SUPPRESS_DUPLICATE_TAG = {
  tagName: 'suppress',
  text: '{duplicate}'
};
...
class ExternsWriter extends ClosureRewriter {
  ...
  public visit(node: ts.Node, namespace: string[] = []) {
    switch (node.kind) {
      ...
      case ts.SyntaxKind.ModuleDeclaration:
        ...
        switch (decl.name.kind) {
          case ts.SyntaxKind.Identifier:
            ...
            if (name === 'global') {
              ...
              if (this.isFirstDeclaration(decl)) {
                this.writeExternsVariable(name, namespace, [{tagName: 'const'}], '{}');
              }
              namespace = namespace.concat(name);
            }
          ...
          case ts.SyntaxKind.StringLiteral:
            ...
            this.writeExternsVariable('tsickle_declare_module', [], [{tagName: 'const'}], '{}');
            ...
            if (this.isFirstDeclaration(decl)) {
              this.writeExternsVariable(importName, namespace, [{tagName: 'const'}], '{}');
            }
      ...
      case ts.SyntaxKind.FunctionDeclaration:
        ...
        const params = this.emitFunctionType(decls, [SUPPRESS_DUPLICATE_TAG]);
  ...
  private writeExternsType(decl: ts.InterfaceDeclaration|ts.ClassDeclaration, namespace: string[]) {
    ...
    if (this.isFirstDeclaration(decl)) {
      ...
      if (decl.kind === ts.SyntaxKind.ClassDeclaration) {
        ...
        if (ctors.length) {
          // Don't emit jsdoc here, as emitFunctionType will do it.
          writeJsDoc = false;
          ...
          if (ctors.length > 1) {
            paramNames = this.emitFunctionType(
                ctors as ts.ConstructorDeclaration[], [...ctorTags, SUPPRESS_DUPLICATE_TAG]);
          } else {
            paramNames = this.emitFunctionType([firstCtor], [...ctorTags, SUPPRESS_DUPLICATE_TAG]);
          }
        }
      } else {
      ...
      if (writeJsDoc) {
        jsdocTags.push(SUPPRESS_DUPLICATE_TAG);
  ...
  private writeExternsVariableDecl(decl: ts.VariableDeclaration, namespace: string[]) {
    if (decl.name.kind === ts.SyntaxKind.Identifier) {
      ...
      const type = this.typeToClosure(decl);
      this.writeExternsVariable(name, namespace, [{tagName: 'type', type}]);
    } else {
      this.errorUnimplementedKind(decl.name, 'externs for variable');
    }
  }

  private writeExternsVariable(
      name: string, namespace: string[], tags: jsdoc.Tag[] = [], value?: string) {
    this.emit(jsdoc.toString([...tags, SUPPRESS_DUPLICATE_TAG]));
  ...
  private writeExternsEnum(decl: ts.EnumDeclaration, namespace: string[]) {
    ...
    for (const member of decl.members) {
      ...
      this.writeExternsVariable(memberName, namespace, [{tagName: 'const', text: '{number}'}]);
    }
  }

  private writeExternsTypeAlias(decl: ts.TypeAliasDeclaration, namespace: string[]) {
    const typedefTag = {tagName: 'typedef', text: `{${this.typeToClosure(decl)}}`};
    this.writeExternsVariable(getIdentifierText(decl.name), namespace, [typedefTag]);
  }
}
