 ...
 * system, ...).
 */
const FILEOVERVIEW_COMMENT_MARKERS: ReadonlySet<string> = new Set([
  'fileoverview',
  'externs',
  'modName',
  'mods',
...
export function transformFileoverviewCommentFactory(
  ...
  return (): ((sourceFile: ts.SourceFile) => ts.SourceFile) => {
    function checkNoFileoverviewComments(
      context: ts.Node,
      comments: jsdoc.SynthesizedCommentWithOriginal[],
      message: string,
    ) {
    ...
    return (sourceFile: ts.SourceFile) => {
      ...
      for (let i = fileComments.length - 1; i >= 0; i--) {
        ...
        if (
          parsed !== null &&
          parsed.tags.some((t) => FILEOVERVIEW_COMMENT_MARKERS.has(t.tagName))
        ) {
          fileoverviewIdx = i;
          tags = parsed.tags;
          break;
