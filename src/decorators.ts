 ...
 * in its JSDoc).
 */
export function hasExportingDecorator(
  node: ts.Node,
  typeChecker: ts.TypeChecker,
) {
 ...
 */
function isExportingDecorator(
  decorator: ts.Decorator,
  typeChecker: ts.TypeChecker,
) {
  return getDecoratorDeclarations(decorator, typeChecker).some(
    (declaration) => {
...
export function transformDecoratorsOutputForClosurePropertyRenaming(
  ...
  return (context: ts.TransformationContext) => {
    const result: ts.Transformer<ts.SourceFile> = (
      ...
      if (nodeNeedingGoogReflect !== undefined) {
        ...
        statements.splice(
          googModuleIndex + 3,
          0,
          googRequireReflectObjectProperty,
        );
 ...
 */
function sanitizeDecorateComments(
        ...
        if (
          ...
          if (!comments || comments.length === 0) {
            ...
            ts.setSyntheticLeadingComments(
              stmt,
              sanitizeDecorateComments(comments),
            );
          }
        }
      }
