...
function compareAgainstGolden(output: string|null, path: string) {
  ...
  if (UPDATE_GOLDENS && output !== golden) {
    console.log(`Updating golden path file for ${path} with ${patchPath}`);
    if (output !== null) {
      const patchOutput =
    ...
    expect(output).to.equal(golden, `${path} with ${patchPath}`);
  }
}
...
function compareAgainstGoldenDiagnostics(diagnostics: ts.Diagnostic[], path: string) {
  ...
  const golden = readGolden(tsicklePath) || '';
...
testFn('golden transformer tests', () => {
  testSupport.goldenTests().forEach((test) => {
    ...
    it(test.name, () => {
      ...
      const program = testSupport.createProgram(tsSources, tsHost, tsCompilerOptions);
      {
